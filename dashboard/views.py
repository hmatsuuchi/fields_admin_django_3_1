...
from authentication.permissions import isInStaffGroup, isInSuperusersGroup
...
class AtRiskStudentsView(APIView):
    ...
    def get(self, request, format=None):        
        try:
            ...
            at_risk_students = StudentChurnPrediction.objects.filter(churn_probability__gte=0.6).order_by('-churn_probability')
...
# analyze attendance
class AttendanceForAllInstructorsView(APIView):
    authentication_classes = ([CustomAuthentication])
    ...
    def get(self, request, format=None):
        try:
            ...
            while most_recent_sunday.weekday() != 6: # Sunday=6
                most_recent_sunday -= timedelta(days=1)

            # set the date range for analysis always starting on Monday and ending on Sunday
            range_start_date = most_recent_sunday - timedelta(weeks=analysis_date_range)
            range_end_date = most_recent_sunday - timedelta(days=1)
            ...
            for instructor in instructor_list:
                ...
                for day_offset, day_of_week in enumerate(days_of_week):
                    date = range_start_date + timedelta(days=day_offset) # first day of week in date range
